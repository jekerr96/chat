...
$result = mysqli_query($link, "SELECT COUNT(*) as count, CAST(visit_date AS DATE) as visit_date FROM stat GROUP BY CAST(visit_date AS DATE)");
while( $row = mysqli_fetch_assoc($result) ){ 
            $myData->addPoints($row['count'], "Посещения");
...
$myData->setPalette("Посещения",$serieSettings);
